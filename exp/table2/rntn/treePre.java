...
public class treePre {
    ...
    public static void processTweet(String input, String output) throws FileNotFoundException {
        ...
        while (sc.hasNextLine()) {
                ...
                Scanner tweetSc = new Scanner (line);
                ...
                if (tweetSc.hasNextLine()) {
											 String tweet = tweetSc.nextLine().trim();
											 char last = tweet.charAt(tweet.length() - 1);
                       out.print(tweet);
											 if (lineEnd.indexOf(last) < 0)
                           out.print(".");
                }
