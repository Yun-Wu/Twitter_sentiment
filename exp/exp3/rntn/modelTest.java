...
public class modelTest {
	...
	public static final int NET = 4;
  	...
  	public static boolean hardD = false; 
    	...
    	Input inputFormat = Input.HARD;
  		...
  		for (int argIndex = 0; argIndex < args.length; ) {
      		if (args[argIndex].equalsIgnoreCase("-fileG")) {
        		...
        		binary = true;
        		argIndex++;
        	} else if (args[argIndex].equalsIgnoreCase("-hardD")) {
        		hardD = true;
        		argIndex++;
	...
	public static int checkOne(Scanner input, Scanner parsed, Input inputFormat) {
		...
		String pText = "";
		double[] softVector = new double[5];  // probability of the whole tweet
		int[] hardVector = new int[2];        // sentiment count of the whole tweet [pos, neg]
		...
		switch (inputFormat) {
			...
			case SOFT:
			    do {
						...
						parseProb(pLine, parsed, softVector, hardVector);
				    } while (!text.endsWith(pText));
				parsedSentiment = getProbSentiment(softVector, hardVector);
				break;
			default:
	...
	public static int getProbSentiment(double[] prob, int[] count) {
		...
		if (binary) {
			triVec[0] += triVec[1] * 0.4;
			triVec[2] += triVec[1] * 0.6;
			triVec[1] = 0;
		}
        ...
        // if just make hard decision
		if (hardD && count[0] != count[1]) {
			if (count[0] > count[1]) return POS;
			else return NEG;
		}
	...
	public static void parseProb (String treeLine, Scanner input, double[] softV, int[] hardV) {
        ...
        // get per sentence hard decision
		if (prob[0] + prob[1] < prob[3] + prob[4] +  prob[2])
				hardV[0]++;
		else 
				hardV[1]++;
    ...
    public static void help() {
    	...
    	System.err.println("  -hardD: make hard decision on combining");
  }
